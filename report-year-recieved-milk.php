                ...
                <table class="table table-bordered text-center" id="staff-table">
                        ...
                        <?php 
                            while (strtotime($from_month) < strtotime($tomonth)) {
                                ...
                                for($i=0; $i<sizeof($milk_provder_arr);$i++){
                                    ...
                                    $query2 = "select sum(trm.quantity) total_quantity, sum(trm.quantity)*tmp.rate tamount from tbl_recieved_milk trm inner join tbl_milk_provider tmp on trm.milk_provider_id = tmp.id where trm.milk_provider_id=$id and DATE_FORMAT(cast(trm.date_time as date), '%Y-%m') BETWEEN '$from_month' AND '$tomonth'";
                    ...
                    <tfoot>
                        ...
                        <?php
                            ...
                            $query3 = "select trm.milk_provider_id id, sum(trm.quantity)*tmp.rate tamount, sum(trm.quantity) total_quantity from tbl_recieved_milk trm left join tbl_milk_provider tmp on trm.milk_provider_id = tmp.id where DATE_FORMAT(cast(trm.date_time as date), '%Y-%m') BETWEEN '$from_month2' and '$tomonth2' group by trm.milk_provider_id order by trm.milk_provider_id";
