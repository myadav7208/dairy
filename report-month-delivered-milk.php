...
?>
        ...
        <?php
            ...
            <div class="table-responsive" style="max-width:1000px;min-width:1000px;">
                <table class="table table-bordered text-center " id="staff-table" >
                        ...
                        <?php 
                            while (strtotime($from_date) <= strtotime($todate)) {
                                ...
                                for($i=0; $i<sizeof($milk_provder_arr);$i++){
                                    ...
                                    $query2 = "select sum(trd.quantity) total_quantity, sum(trd.quantity)*tc.rate tamount from tbl_milk_delivered trd inner join tbl_client tc on trd.client_id = tc.id where trd.client_id=$id and cast(trd.date_time as date) like '%$from_date%'";
                    ...
                    <tfoot>
                            ...
                            <?php
                                ...
                                $query3 = "select tmd.client_id id, sum(tmd.quantity)*tc.rate tamount, sum(tmd.quantity) total_quantity from tbl_milk_delivered tmd left join tbl_client tc on tmd.client_id = tc.id where DATE_FORMAT(cast(tmd.date_time as date), '%Y-%m-%d') BETWEEN '$from_date2' and '$todate2' group by tmd.client_id order by tmd.client_id";
